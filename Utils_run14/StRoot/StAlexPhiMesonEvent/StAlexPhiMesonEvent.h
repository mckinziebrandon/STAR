...
public:
    StAlexPhiMesonTrack() :
        ...
        rap(-1),phi(-1),theta(-1),Psi_ep(-1),Psi_ep_eta(-1),Psi_diff_ME(-1), Delta_dip_angle(-1), qpA(-1), qpB(-1)
    {
    }
        ~StAlexPhiMesonTrack() {}
        ...
        void setDelta_dip_angle(Float_t f)         { Delta_dip_angle = f; }
        ...
        Float_t getDelta_dip_angle() const         { return Delta_dip_angle; }
...
public:
        ...
        EP_Qx_ptw(-1),EP_Qy_ptw(-1),Qtracks_eta_pos(0),Qtracks_eta_neg(0),Qtracks_full(0),ZDCx(-1),BBCx(-1),vzVpd(-1),fNumTracks(0)
    {
        ...
        StAlexPhiMesonTrack* createTrack()
        {
            ...
            if (fNumTracks >= 10000)
            {
                Fatal( "StAlexPhiMesonEvent::createTrack()", "ERROR: Too many tracks (>1000)!" );
                exit( 2 );
            }
