...
void StJetSimAnalysis::Make()
{
    ...
    Double_t phi_acc[4][2] = {{0.0,    1.0*Pi},  
                             {1.0*Pi, 2.0*Pi},  
                             {0.5*Pi, 1.5*Pi},  
                             {0.0,    2.0*Pi}}; 
    Double_t eta_acc[4][2] = {{-1.0,   1.0},
                             {0.0,    1.0},
                             {-0.5,   0.5}, 
                             {-1.0,   1.0}};
    ...
    for (Int_t i_acc = 0; i_acc < 4; i_acc++)
    {
        // ---------------------------------------- begin event loop ----------------------------------------
        for(Long64_t n_evt = eStartEvent; n_evt < (eStartEvent+N_Events); n_evt++)
        {
            ...
            // declare event jet container and trgger tracks
            ...
            for(Int_t n_track = 0; n_track < N_Tracks; n_track++) 
            {
                ...
                if(Delta_phi_angle < -TMath::Pi()) Delta_phi_angle += 2.0*TMath::Pi();
                if(i_acc == 0) h_Delta_phi ->Fill(Delta_phi_angle);
                if(i_acc == 0) h_track_pt_vs_delta_phi ->Fill(Delta_phi_angle,pT_track);
            ...
            if(i_acc == 0) h_trigger_pt_vs_delta_phi   ->Fill( vec_trigger_tracks[random_trigger_index][3],
                                                vec_trigger_tracks[random_trigger_index][2]);
            if(i_acc == 0) h_Delta_phi_trigger         ->Fill( vec_trigger_tracks[random_trigger_index][3]);
            //----------------------------------------------------------------------------------
            ...
            // Start of jet reconstruction
                ...
                Selector Fiducial_cut_selector  = SelectorAbsEtaMax(eta_acc[i_acc][hi] - Jet_R);
                ...
                if(vec_trigger_tracks.size() > 0)
                {
                    ...
                    for(Int_t i_jet = 0; i_jet < jets.size(); i_jet++)
                    {
                        ...
                        for (Int_t j = 0; j < jet_constituents.size(); j++)
                        {
                            ...
                            if( jet_delta_eta < jet_delta_eta_cut &&
        ...
        delete NT_ReCoil_Jet;
        delete h_jet_pt_sub;               
        delete h_jet_pt_vs_Delta_phi;      
        delete h_jet_pt_vs_trigger_pt;     
        delete h_jet_recoil_pt_vs_Delta_phi;

        HistName = "h_jet_pt_sub";
        HistName += i_acc+1;
        h_jet_pt_sub                 = new TH1D(HistName.Data(),HistName.Data(),1400,-40,100.0);

        HistName = "h_jet_pt_vs_Delta_phi";
        HistName += i_acc+1;
        h_jet_pt_vs_Delta_phi        = new TH2D(HistName.Data(),HistName.Data(),50,-TMath::Pi(),TMath::Pi(),50,-20,30);

        HistName = "h_jet_pt_vs_trigger_pt";
        HistName += i_acc+1;
        h_jet_pt_vs_trigger_pt       = new TH2D(HistName.Data(),HistName.Data(),50,0,3.5,50,-20,30);

        HistName = "h_jet_recoil_pt_vs_Delta_phi";
        HistName += i_acc+1;
        h_jet_recoil_pt_vs_Delta_phi = new TH2D(HistName.Data(),HistName.Data(),50,-TMath::Pi(),TMath::Pi(),50,-20,30);

        HistName = "NT_ReCoil_Jet";
        HistName += i_acc+1;
        NT_ReCoil_Jet = new TNtuple(HistName.Data(), 
                                    "NT_ReCoil_Jet Ntuple", 
                                    "EventId:JetId:rho:area:Jetphi:Jeteta:Jetpt:TrackId:eta:phi:pt:x:y:z");
        NT_ReCoil_Jet->SetAutoSave( 5000000 );
    } 
