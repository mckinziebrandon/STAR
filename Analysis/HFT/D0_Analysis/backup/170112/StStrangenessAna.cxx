...
static const Int_t N_Analysis = 6;
...
void StStrangenessAna::init()
{
    ...
    if (!SEList.IsNull())   // if input file is ok
    {
        ...
        if(in)
        {
            ...
            input_SE  = new TChain( ALEXPHIMESON_EVENT_TREE, ALEXPHIMESON_EVENT_TREE );
            ...
            while(in)
            {
                ...
                if(str[0] != 0)
                {
                    ...
                    input_SE ->AddFile(addfile.Data(),-1, ALEXPHIMESON_EVENT_TREE );
    ...
    // Set the input tree
    if (!input_SE->GetBranch( ALEXPHIMESON_EVENT_BRANCH ))
    {
        cerr << "ERROR: Could not find branch '"
            << ALEXPHIMESON_EVENT_BRANCH << "'in tree!" << endl;
    }

    alexPhiMeson_event = new StAlexPhiMesonEvent();
    input_SE  ->SetBranchAddress( ALEXPHIMESON_EVENT_BRANCH, &alexPhiMeson_event );
    ...
    if (!MEList.IsNull())   // if input file is ok
    {
        ...
        {
            cout << "file list is ok" << endl;
            input_ME  = new TChain( ALEXPHIMESON_EVENT_TREE, ALEXPHIMESON_EVENT_TREE );
            char str[255];       // char array for each file name
            Long64_t entries_save = 0;
            while(in)
            {
                ...
                if(str[0] != 0)
                {
                    ...
                    addfile = str;
                    addfile = pinputdir+addfile;
                    input_ME ->AddFile(addfile.Data(),-1, ALEXPHIMESON_EVENT_TREE );
    ...
    // Set the input tree
    if (!input_ME->GetBranch( ALEXPHIMESON_EVENT_BRANCH ))
    {
        cerr << "ERROR: Could not find branch '"
            << ALEXPHIMESON_EVENT_BRANCH << "'in tree!" << endl;
    }
    ...
    Int_t   nAna_InvMass_bins[N_Analysis]  = {200,200,200,200,200,200};
    Float_t nAna_InvMass_start[N_Analysis] = {0.98,0.98,0.98,0.98,0.98,0.98};
    Float_t nAna_InvMass_stop[N_Analysis]  = {1.16,1.16,1.16,1.16,1.16,1.16};
...
void StStrangenessAna::loop()
{
    ...
    for(Int_t SE_ME_loop = 0; SE_ME_loop < 2; SE_ME_loop++) // 0 = same event, 1 = mixed event
    {
        ...
        if(SE_ME_loop == 1)
        {
            input_ME  ->SetBranchAddress( ALEXPHIMESON_EVENT_BRANCH, &alexPhiMeson_event );
            cout << "" << endl;
        ...
        for(Long64_t counter = start_event_use; counter < stop_event_use; counter++)
        {
            ...
            if (counter != 0  &&  counter % 10000 == 0)
            {
                if((stop_event_use-start_event_use) > 0)
                {
                    ...
                    cout << " " << counter << " (" << event_percent << "%) " << "\n" << "==> Processing data (hadron_v2) " << flush;
                }
            }
            ...
            Int_t   erun_nevents         = alexPhiMeson_event->getNumTracks(); // number of tracks in this event
            Float_t EventVertexX         = alexPhiMeson_event->getx();
            Float_t EventVertexY         = alexPhiMeson_event->gety();
            Float_t EventVertexZ         = alexPhiMeson_event->getz();
            Int_t   RunId                = alexPhiMeson_event->getid();
            Float_t refMult              = alexPhiMeson_event->getmult();
            Int_t   n_prim               = alexPhiMeson_event->getn_prim();
            Int_t   n_non_prim           = alexPhiMeson_event->getn_non_prim();
            Int_t   n_tof_prim           = alexPhiMeson_event->getn_tof_prim();
            Float_t EP_Qx_eta_pos_ptw    = alexPhiMeson_event->getEP_Qx_eta_pos_ptw();
            Float_t EP_Qy_eta_pos_ptw    = alexPhiMeson_event->getEP_Qy_eta_pos_ptw();
            Float_t EP_Qx_eta_neg_ptw    = alexPhiMeson_event->getEP_Qx_eta_neg_ptw();
            Float_t EP_Qy_eta_neg_ptw    = alexPhiMeson_event->getEP_Qy_eta_neg_ptw();
            Float_t EP_Qx_ptw            = alexPhiMeson_event->getEP_Qx_ptw();
            Float_t EP_Qy_ptw            = alexPhiMeson_event->getEP_Qy_ptw();
            Int_t   Qtracks_eta_pos      = alexPhiMeson_event->getQtracks_eta_pos();
            Int_t   Qtracks_eta_neg      = alexPhiMeson_event->getQtracks_eta_neg();
            Int_t   Qtracks_full         = alexPhiMeson_event->getQtracks_full();
            Float_t ZDCx                 = alexPhiMeson_event->getZDCx();
            Float_t BBCx                 = alexPhiMeson_event->getBBCx();
            Float_t vzVpd                = alexPhiMeson_event->getvzVpd();
              ...
              )
            {
                for(UShort_t i = 0; i < erun_nevents; ++i) // loop over all tracks of the actual event
                {
                    ...
                    Float_t m2A              = alexPhiMeson_track->getm2A();
                    Float_t m2B              = alexPhiMeson_track->getm2B();
                    Float_t nsA              = alexPhiMeson_track->getnsA();
                    Float_t nsB              = alexPhiMeson_track->getnsB();
                    Float_t dcaA             = alexPhiMeson_track->getdcaA();
                    Float_t dcaB             = alexPhiMeson_track->getdcaB();
                    Float_t iQxA             = alexPhiMeson_track->getiQxA();
                    Float_t iQyA             = alexPhiMeson_track->getiQyA();
                    Float_t iQxB             = alexPhiMeson_track->getiQxB();
                    Float_t iQyB             = alexPhiMeson_track->getiQyB();
                    Float_t etaA             = alexPhiMeson_track->getetaA();
                    Float_t etaB             = alexPhiMeson_track->getetaB();
                    Float_t InvAB            = alexPhiMeson_track->getInvAB();
                    Float_t p_t              = alexPhiMeson_track->getpt();
                    Float_t rap              = alexPhiMeson_track->getrap(); // true Rapidity, not pseudo-Rapidity!
                    Float_t phi              = alexPhiMeson_track->getphi();
                    Float_t theta            = alexPhiMeson_track->gettheta();
                    Float_t phi_event_plane  = alexPhiMeson_track->getPsi_ep();
                    Float_t phi_event_plane_eta_gap = alexPhiMeson_track->getPsi_ep_eta();
                    Float_t Psi_diff_ME      = alexPhiMeson_track->getPsi_diff_ME();
                    Float_t Delta_dip_angle  = alexPhiMeson_track->getDelta_dip_angle();
                    Float_t qpA              = alexPhiMeson_track->getqpA();
                    Float_t qpB              = alexPhiMeson_track->getqpB();
                    ...
                    if(fabs(etaB) < eta_EP_cut && dcaB < dcaAB_EP_cut &&
                       fabs(qpB) > MomentumA_EP_low_cut && fabs(qpB) < MomentumA_EP_high_cut)
                    {
                    ...
                    {
                        phi_event_plane_eta_pos_gap_rc = calc_phi_event_plane_2nd(EP_Q_vectors_rc[0]-(flag_weight_part_A_EP[0]*iQxA+flag_weight_part_B_EP[0]*iQxB)/N_tracks_EP_Q_vectors[0],EP_Q_vectors_rc[1]-(flag_weight_part_A_EP[0]*iQyA+flag_weight_part_B_EP[0]*iQyB)/N_tracks_EP_Q_vectors[0]); //
                        phi_event_plane_eta_neg_gap_rc = calc_phi_event_plane_2nd(EP_Q_vectors_rc[2]-(flag_weight_part_A_EP[1]*iQxA+flag_weight_part_B_EP[1]*iQxB)/N_tracks_EP_Q_vectors[2],EP_Q_vectors_rc[3]-(flag_weight_part_A_EP[1]*iQyA+flag_weight_part_B_EP[1]*iQyB)/N_tracks_EP_Q_vectors[2]); //
                    }
                    if(N_tracks_EP_Q_vectors[4] > 0.0 && N_tracks_EP_Q_vectors[5] > 0.0)
                    {
                        phi_event_plane_rc             = calc_phi_event_plane_2nd(EP_Q_vectors_rc[4]-(flag_weight_part_A_EP[2]*iQxA+flag_weight_part_B_EP[2]*iQxB)/N_tracks_EP_Q_vectors[4],EP_Q_vectors_rc[5]-(flag_weight_part_A_EP[2]*iQyA+flag_weight_part_B_EP[2]*iQyB)/N_tracks_EP_Q_vectors[5]); // subtract the Q-vector of the track to avoid auto correlations
                    }
                      ...
                      )
                    {
                          ...
                          )
                        {
                            ...
                            {
                                hInvMass_pt_cuts[SE_ME_loop][ept_bin][3] ->Fill(InvAB,reweight); // best
                            }
                        }
                        if(
                           (p_t < 1.2 || (p_t >= 1.2 && ( (m2A > 0.16 && m2A < 0.36) || (m2B > 0.16 && m2B < 0.36) ) )) &&
                          ...
                          )
                        {
                            if(
                               ((m2A < -10 && nsA < 2.5 && nsA > -1.2) || (m2A > 0.16 && m2A < 0.36)) &&
                               ((m2B < -10 && nsB < 2.5 && nsB > -1.2) || (m2B > 0.16 && m2B < 0.36)) &&
                               Delta_dip_angle > 0.04 && Delta_dip_angle < 1.3
                              )
                            {
                                hInvMass_pt_cuts[SE_ME_loop][ept_bin][5] ->Fill(InvAB,reweight);
                            }
                        }
                        if(
                           (p_t < 1.2 || (p_t >= 1.2 && ( (m2A > 0.16 && m2A < 0.36) || (m2B > 0.16 && m2B < 0.36) ) )) &&
                          ...
                          )
                        {
                            if(
                               ((m2A < -10 && nsA < 2.5 && nsA > -1.2) || (m2A > 0.16 && m2A < 0.36)) &&
                               ((m2B < -10 && nsB < 2.5 && nsB > -1.2) || (m2B > 0.16 && m2B < 0.36)) &&
                               Delta_dip_angle > 0.06
                              )
                            {
                                hInvMass_pt_cuts[SE_ME_loop][ept_bin][6] ->Fill(InvAB,reweight);
                            }
                        }
                        if(
                           (p_t < 1.2 || (p_t >= 1.2 && ( (m2A > 0.16 && m2A < 0.36) || (m2B > 0.16 && m2B < 0.36) ) )) &&
                          ...
                          )
                        {
                              ...
                              )
                            {
                                ...
                                hInvMass_pt_cuts[SE_ME_loop][ept_bin][1] ->Fill(InvAB,reweight); // best cut
