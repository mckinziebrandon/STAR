...
{
    Int_t N_points = 0;
    for(Int_t i_point = 1; i_point < Histo->GetN(); i_point++)
    {
        Double_t x,y;
...
void Calc_syst_error_graph_hist::calculate_error()
{
    ...
    for(Int_t i_point = start_point; i_point < stop_point; i_point++)
    {
        ...
        if(flag_add_stat_error) tgae_stat->SetPointError(i_point,x_err_low,x_err_high,0.0,0.0);
    }
}
...
public:
    ...
    Double_t get_RMS();
    std::vector<Double_t> vec_RMS;
    std::vector<Double_t> vec_RMS_trunc;
};
    ...
    if(RMS >= 0.0)
    {
        for(Int_t i_MC = 0; i_MC < N_MC; i_MC++)
        {
            ...
            if(val_den > 0.0 && val_nom > 0.0) // take only positive values (yields)
            {
                ...
                Double_t RMS_val   = ratio_nom_den - ratio_val;
                vec_RMS.push_back(RMS_val);
                RMS += RMS_val*RMS_val;
#if 0
                cout << "i_MC: " << i_MC << ", val_nom: " << val_nom << ", nom_err: " << nom_err << ", val_den: " << val_den << ", den_err: " << den_err
                    << ", ratio_val: " << ratio_val << ", N_ratios: " << N_ratios
                    << ", RMS_val: " << RMS_val << ", RMS: " << RMS << endl;
#endif
            }
        }
        if(vec_RMS.size() > 0)
        {
            RMS = TMath::Sqrt((1.0/((Double_t)vec_RMS.size()))*RMS);
        }
        else RMS = -1;
    }
    //cout << "RMS: " << RMS << endl;
...
{
    for(Int_t i_entry = 0; i_entry < vec_RMS.size(); i_entry++)
    {
        if( fabs(vec_RMS[i_entry] - ratio_nom_den) < nSigma*RMS)
        {
            vec_RMS_trunc.push_back(vec_RMS[i_entry]);
        }
    }
    RMS = 0.0;
    vec_RMS.clear();
    for(Int_t i_entry = 0; i_entry < vec_RMS_trunc.size(); i_entry++)
    {
        RMS += vec_RMS_trunc[i_entry]*vec_RMS_trunc[i_entry];
        vec_RMS.push_back(vec_RMS_trunc[i_entry]);
    }
    if(vec_RMS_trunc.size() > 0)
    {
        RMS = TMath::Sqrt((1.0/((Double_t)vec_RMS_trunc.size()))*RMS);
    }
    else RMS = -1;
    vec_RMS_trunc.clear();
}

Double_t Monte_Carlo_Error_Ratio::get_RMS()
{
    vec_RMS.clear();
    vec_RMS_trunc.clear();
    return RMS;
